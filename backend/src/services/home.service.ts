...
export class HomeService
{
    ...
    {
        const [row] = await db.query('SELECT Lista_Deseo.fk_id_producto AS "id", Producto.Nombre AS "name", Producto.Precio AS "price", Imagen.Nombre AS "Imagen" FROM Lista_Deseo JOIN Usuario ON Lista_Deseo.fk_id_usuario = Usuario.id JOIN Producto ON Lista_Deseo.fk_id_producto = Producto.id JOIN Imagen on Imagen.fk_id_producto = Producto.id WHERE Lista_Deseo.fk_id_usuario = ?', [id]);
