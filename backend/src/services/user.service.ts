...
export class UserService{
    ...
    {
        const [ row ] = await db.query(`SELECT Producto.id, Producto.Nombre AS 'published', DATE_FORMAT(Producto.Fecha_Publicacion, '%y-%m-%d') AS 'date', Imagen.Nombre AS 'image' FROM Producto JOIN Usuario ON Producto.fk_id_usuario = Usuario.id JOIN Imagen on Imagen.fk_id_producto = Producto.id WHERE Producto.fk_id_usuario = ? AND Producto.Disponibilidad = 'Disponible'`, [id]);
